...
export const LongPressButton = ({ onComplete, label = "Hold to Delete", duration = 2000 }) => {
  ...
  useEffect(() => {
    if (isPressing) {
      intervalRef.current = setInterval(() => {
        setProgress((prev) => {
          if (prev >= 100) { clearInterval(intervalRef.current); onComplete(); return 0; }
  ...
  return (
    <button
      ...
      className="relative overflow-hidden bg-red-900/20 text-red-400 border border-red-900/50 px-4 py-2 rounded text-sm font-bold transition-all select-none active:scale-95 flex-1 flex justify-center items-center"
    >
      <div className="absolute left-0 top-0 bottom-0 bg-red-600 transition-all duration-75 ease-linear opacity-20" style={{ width: `${progress}%` }} />
      <span className="relative z-10 flex items-center gap-2">{progress >= 100 ? "DELETING..." : <><Icons.Trash /> {label}</>}</span>
    </button>
  );
};
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <Bar
            ...
            radius={[4, 4, 0, 0]}
          >
            {/* 3. Color Logic: Red (Overdue), Yellow (Due Soon), Green (Good) */}
            {data.map((entry, index) => (
              <Cell
                key={`cell-${index}`}
                fill={entry.remaining < 0 ? '#ef4444' : entry.remaining < 30 ? '#eab308' : '#22c55e'}
              />
            ))}
          </Bar>
        </BarChart>
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        <div
          ...
          className={`h-8 flex flex-col items-center justify-center relative border rounded transition-all ${isSelected ? 'bg-blue-900 border-blue-500 ring-2 ring-blue-400 z-10' : 'border-transparent hover:bg-slate-700'} ${isDue ? 'font-bold cursor-pointer' : ''}`}
          title={isDue ? dayAssets.map(a => a.name).join(', ') : ''}
        >
          <span className={`text-xs ${isDue ? (isOverdue ? 'text-red-400' : 'text-orange-400') : 'text-slate-400'}`}>{d}</span>
          {isDue && (
            <div className="flex gap-0.5 mt-0.5">
              {dayAssets.slice(0, 3).map((_, i) => <div key={i} className={`w-1 h-1 rounded-full ${isOverdue ? 'bg-red-500' : 'bg-orange-500'}`}></div>)}
            </div>
          )}
        </div>
      );
    ...
    <div className="bg-slate-800 rounded-xl shadow-sm border border-slate-700 p-4 mt-6 no-print">
      <div className="flex justify-between items-center mb-4 border-b border-slate-700 pb-2">
        <div className="flex items-center gap-2">
          <span className="text-lg text-blue-400 bg-blue-900/30 p-1 rounded"><Icons.Calendar /></span>
