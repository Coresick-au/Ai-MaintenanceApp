                ...
                <div className="flex-1 overflow-auto bg-slate-800/60 rounded-xl border border-slate-700">
                    <table ref={tableRef} className="text-left text-sm" style={{ tableLayout: 'auto' }}>
                        <thead className="bg-slate-900 text-slate-400 sticky top-0 z-10">
                            <tr>
                                <th className="px-4 py-3 cursor-pointer hover:bg-slate-800 transition-colors relative" onClick={() => handleSort('sku')} style={{ width: `${columnWidths[0]}px` }}>
                                    <div className="flex items-center gap-2 column-content">SKU {getSortIcon('sku')}</div>
                                ...
                                <th className="px-4 py-3 cursor-pointer hover:bg-slate-800 transition-colors relative" onClick={() => handleSort('name')} style={{ width: `${columnWidths[1]}px` }}>
                                    <div className="flex items-center gap-2 column-content">Name {getSortIcon('name')}</div>
                                ...
                                <th className="px-4 py-3 cursor-pointer hover:bg-slate-800 transition-colors relative" onClick={() => handleSort('categoryId')} style={{ width: `${columnWidths[2]}px` }}>
                                    <div className="flex items-center gap-2 column-content">Category {getSortIcon('categoryId')}</div>
                                ...
                                <th className="px-4 py-3 cursor-pointer hover:bg-slate-800 transition-colors relative" onClick={() => handleSort('subcategoryId')} style={{ width: `${columnWidths[3]}px` }}>
                                ...
                                filteredAndSortedFasteners.map(fastener => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-sm text-slate-400">
                                            {fastener.suppliers?.length > 0 ? (
                                                <div className="flex flex-wrap gap-1">
                                                    {fastener.suppliers.slice(0, 2).map((s, i) => (
                                                        <span key={i} className="text-xs text-slate-500 bg-slate-800/50 px-1.5 py-0.5 rounded">
                                                            {s}
                                                        </span>
                                                    ))}
                                                    {fastener.suppliers.length > 2 && (
                                                        <span className="text-xs text-slate-500 px-1">+ {fastener.suppliers.length - 2}</span>
                                                    )}
                                                </div>
                                            ) : '-'}
                                        </td>
