...
export const TemplateEditorModal = () => {
  ...
  const saveTpl = () => {
    ...
    if (editTplId) setTpls(tpls.map(x => x.id === editTplId ? { ...x, ...tplDraft } : x));
    else setTpls([...tpls, { ...tplDraft, id: "tpl_" + uid(), isDefault: false }]);
    setShowTplEd(false);
  };
      ...
      <div style={{ background: t.surface, borderRadius: 10, border: `1px solid ${t.border}`, width: "100%", maxWidth: 800, maxHeight: "85vh", overflow: "auto" }}>
        ...
        <div style={{ padding: 20 }}>
          ...
          <div style={{ background: t.surfaceDeep, borderRadius: 6, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "40px 1fr 140px 40px", padding: "8px 12px", borderBottom: `1px solid ${t.border}` }}>
              {["#", "Parameter Name", "Unit", ""].map(h => <span key={h} style={{ fontSize: 10, fontWeight: 600, color: t.textFaint, textTransform: "uppercase" }}>{h}</span>)}
            </div>
            {tplDraft.params.map((p, i) => (
              <div key={p.id} style={{ display: "grid", gridTemplateColumns: "40px 1fr 140px 40px", padding: "4px 12px", borderBottom: `1px solid ${t.borderSub}`, alignItems: "center" }}>
